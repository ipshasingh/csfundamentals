...
class HypercubeVisualizer(QMainWindow):
    ...
    def create_experiments_page(self):
        ...
        subtitle = QLabel(
            "Compare how BFS, DFS and A* behave as hypercube dimension increases."
        )
        ...
        self.path_chart_card, self.path_ax, self.path_canvas = self.create_chart_card(
            "Path Length vs Dimension",
            "Path Length",
            log_y=False
        )
        self.explored_chart_card, self.explored_ax, self.explored_canvas = self.create_chart_card(
        ...
        self.results_table.setHorizontalHeaderLabels([
            "n", "States",
            "BFS Path", "BFS Explored",
            "DFS Path", "DFS Explored",
            "A* Path", "A* Explored"
        ])
    ...
    def run_experiments(self):
        # Keep the experiment deterministic and aligned with the
        # search implementation used by the visualizer.
        rows = []
        ...
        for n in range(2, 11):
            ...
            bfs_path, bfs_visited, _ = bfs_with_events(graph, start, goal)
            dfs_path, dfs_visited, _ = dfs_with_events(graph, start, goal)
            astar_path, astar_visited, _ = a_star_with_events(
                graph, start, goal
            )
        ...
        self.populate_results_table(rows)
        self.plot_experiment_results(rows)

    def populate_results_table(self, rows):
    ...
    def plot_experiment_results(self, rows):
        ...
        self.path_ax.plot(
            dimensions,
            [r["bfs_path"] for r in rows],
            marker="o",
            label="BFS"
        )
        self.path_ax.plot(
            dimensions,
            [r["dfs_path"] for r in rows],
            marker="o",
            label="DFS"
        )
        self.path_ax.plot(
            dimensions,
            [r["astar_path"] for r in rows],
            marker="o",
            label="A*"
        )
        ...
        self.explored_ax.plot(
            dimensions,
            [r["bfs_explored"] for r in rows],
            marker="o",
            label="BFS"
        )
        self.explored_ax.plot(
            dimensions,
            [r["dfs_explored"] for r in rows],
            marker="o",
            label="DFS"
        )
        self.explored_ax.plot(
            dimensions,
            [r["astar_explored"] for r in rows],
            marker="o",
            label="A*"
        )
        self.explored_ax.set_xlabel("Dimension (n)")
        self.explored_ax.set_ylabel("States Explored")
